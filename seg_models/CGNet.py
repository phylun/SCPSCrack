...
class CGNet(nn.Module):
    ...
    def __init__(self, n_classes=19, M=3, N=21, dropout_flag=False):
        """
        args:
        ...
        if dropout_flag:
            ...
            self.classifier = nn.Sequential(nn.Dropout2d(0.1, False), Conv(256, n_classes, 1, 1))
        else:
            self.classifier = nn.Sequential(Conv(256, n_classes, 1, 1))
...
if __name__ == "__main__":
    ...
    model = CGNet(n_classes=2)
    # model = DeepLab(backbone='resnet', output_stride=16)
    # model = DeepLab(backbone='xception', output_stride=16)
    model.eval()
    model.cuda(0)
    summary(model, (3, in_size, in_size))
    img = np.random.randn(3 * in_size * in_size)

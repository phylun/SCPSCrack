...
from seg_models.backbones import PoolFormer
from seg_models.heads import FPNHead
...
class Poolformer(nn.Module):
    ...
    def __init__(self, backbone_type='S24', num_classes=19) -> None:
        super().__init__()
        # super().__init__(backbone, num_classes)
        self.backbone = PoolFormer(backbone_type)
        self.decode_head = FPNHead(self.backbone.channels, 128, num_classes=num_classes)        
...
if __name__ == '__main__':
    model = Poolformer('M36', num_classes=2)
    model.eval()
